...
Le profil Maven `graphrag` déclare `jena-graphrag`, puis `jena-graphrag-integration-tests`. Ce dernier contient son `pom.xml`, un corpus RDF et PDF versionné sous `src/test/resources/corpus/`, un manifeste de provenance/licence/SHA-256 et des validations JUnit de ce socle. Les entrées PDF ajoutées depuis le corpus utilisateur portent une licence `unverified-by-request`. La suite qualifie désormais l'import RDF de production, l'ingestion PDF native, l'index Lucene de texte et la récupération locale avec le corpus. Un Fuseki réel embarqué qualifie aussi `/$/ping`, les contrats JSON de configuration et contexte, les erreurs de précondition des routes GraphRAG et la désactivation complète des routes GraphRAG. Les fournisseurs réels, Playwright et l'orchestration Make restent à réaliser.
