...
The Playwright scenario waits for the task, verifies exactly 12 PDF source files in the dataset, applies a corpus-specific system prompt, and submits five independent chat questions. Each answer must be non-empty and cite an ingested PDF chunk, without asserting provider-specific wording. It makes external embedding and chat calls, so it can take several minutes and incurs the provider's normal usage cost. The prompt and provider values remain process-local and are never emitted in API responses, logs, or Playwright artifacts. See [the step-by-step tutorial](docs/guides/tutoriel/README.md) for the manually orchestrated equivalent.
