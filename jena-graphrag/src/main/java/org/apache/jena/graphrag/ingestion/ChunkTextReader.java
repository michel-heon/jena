...
/** Reads textual GraphRAG chunks in deterministic URI order. */
final class ChunkTextReader {
    private ChunkTextReader() {}

    static List<ChunkText> read(Dataset dataset) {
        ...
        try (QueryExecution execution = QueryExecutionFactory.create(query, dataset)) {
            execution.execSelect().forEachRemaining(solution -> chunks.add(toChunkText(solution)));
        } finally {
            dataset.end();
        }
    ...
    private static ChunkText toChunkText(QuerySolution solution) {
        return new ChunkText(solution.getResource("chunk").getURI(), solution.getLiteral("text").getString());
    }

    record ChunkText(String uri, String text) {}
}